...
$conn = mysql_connect($mysql_server_name, $mysql_username,$mysql_password,'utf-8');
 if (!$conn) {
    die('<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />对不起，数据库的用户或密码不对，请仔细检查。。。。。。具体看官方教程。sorry,localhost,usename or ps maybe is wrong....Could not connect:<br> pls visit later');
    }  
    // mysql_error()
mysql_select_db($mysql_database, $conn) or die ('<meta http-equiv="Content-Type" content="text/html; charset=utf-8" /> 出错，数据库可能不存在。请先用phpmyadmin导入sql文件。或者仔细检查数据库名称。。。。。。。具体看官方教程。sorry,database name maybe is wrong:<br> pls visit later');
