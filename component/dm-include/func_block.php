...
function dmlogin(){
 ...
 //if($curstyle==$curstylenow){ 	  -- no use
...
//mod_regcommon/mod_regcommon.php?lang=cn&type=common&pidname=regcommon20160509_1200413359&file=subaddedit&act=edit&tid=109
	...
	if($type=='video'){ 		 
		...
		if($arr<>'no'){
		$type=$arr['type'];$pid=$arr['pid'];$tid=$arr['id'];
		//if type is page or node,then link jump judge in  mod_video.php.
//mod_video/mod_video.php?lang=cn&pid=page20150805_1138522811&type=page&act2=headless&pidname=video20180126_1915093003&act=edit
		  if($type=='block') $act2='';
		 else $act2='headless';
 $linkv = $urlhere.'/mod_video/mod_video.php?lang='.LANG.'&pidname='.$pidname.'&act2='.$act2.'&act=edit&tid='.$tid.'&pid='.$pid.'&type='.$type;
	 ...
	 if($type=='album'){ 
 		 	...
 		 	//mod_album/mod_mainalbum.php?lang=cn&pid=block&type=block
		   $arr =  get_fieldarr(TABLE_ALBUM,$pidname,'pidname');
		   	if($arr<>'no'){	 
		 $type=$arr['type'];$pid=$arr['pid'];	
         ////if type is page or node,then link jump judge in  mod_mainalbum.php.
////mod_album/mod_album.php?lang=cn&pid=block&type=block&act2=&ppid=album20180125_1910288157&act2=headless
		 if($type=='block') $act2='';
		 else $act2='headless';
	
  $linkv = $urlhere.'/mod_album/mod_album.php?lang='.LANG.'&ppid='.$pidname.'&act2='.$act2.'&pid='.$pid.'&type='.$type;
 ...
 if($type=='form'){ 
 		 ...
 		 //mod_form/mod_field.php?lang=cn&pid=form20180218_1250088582
	
  $linkv = $urlhere.'/mod_form/mod_field.php?lang='.LANG.'&pid='.$pidname;
...
function editlink($v,$type){  //for node content...
global $curstyle;global $curstylenow; 
