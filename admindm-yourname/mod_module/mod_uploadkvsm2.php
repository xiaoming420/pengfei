...
$jumpv = '../mod_module/mod_uploadkvsm2.php?lang='.LANG.'&pidname='.$pidname.'&type='.$type;
...
$sql = "SELECT ppid,kvsm2,sta_orignimg from " . $table . "  where pidname='$pidname' $andlangbh order by id limit 1";
$row =getrow($sql);
if($row<>'no'){
  $imgsqlname  = $row['kvsm2'];  $sta_orignimg  = $row['sta_orignimg'];  $ppid  = $row['ppid'];   
      if($imgsqlname<>''){
          $imgv2 = p2030_imgyt($imgsqlname,'y','n'); //p2030_imgyt($addr,$w_h='y',$linkbig='n')
        //del(actgo2,pidname,backpage,title)
      $delimg= "<br /> <a href=javascript:del('delimg','$pidname','$jumpv','')  class='but2'>删除kvsm2图</a><br /> <br /> ";
      }
...
if($act=="update"){
 ...
 if(!empty($imgname)){
   ...
   $updateimgaddr=", kvsm2='$return_v_s'";
    ...
    jump($jumpv);
       ...
       $ss = "update ".$table." set  kvsm2=''   where pidname='$pidname' $andlangbh limit 1";     
      iquery($ss);
      jump($jumpv);
    } 
