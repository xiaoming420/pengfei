...
<?php
	if($act=='insert'){ 
			if($abc1=="" or strlen($abc1)<3) {alert('请输入分类名称或字太少！');  jump($jumpvf.'&act=add'); }
  ...
  if($act=='add')	{
  ?>
<form  onsubmit="javascript:return catsub(this)" action="<?php  echo $jumpvf.'&act=insert';?>" method="post" enctype="multipart/form-data">
<table class="formtab">
  ...
  <tr>
    ...
    <td width="88%">
	<?php	
 $sql_sel = "select pidname,pid,name,sta_visible from  ".TABLE_CATE." where pid='$pid' and level=1 $andlangbh order by pos desc,id";
