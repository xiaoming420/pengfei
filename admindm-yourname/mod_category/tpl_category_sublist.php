...
?>
 <div class="contenttop">
  <a href="<?php echo $jumpv_back?>&file=subadd&act=add"><i class="fa fa-plus-circle"></i> 添加子分类 </a>
</div>
...
<?php
 ...
 <form method=post action="<?php echo $jumpv_back;?>&act=subpos">
  ...
  <td   align="center">排序号</td>
   ...
   foreach($rowlistsub as $vsub){
        ...
        $edit_desp='<a class=but1 href='.$jumpv_back.'&file=subedit&act=edit&tid='.$vsub['id'].'><i class="fa fa-pencil-square-o"></i> 修改</a>';
		$del_text= " <a href=javascript:del('delsubcate','$pidname','$jumpv_back','$jsname')  class=but2><i class='fa fa-trash-o'></i> 删除</a>";
	//$gltext = 	"<a class=but1 href=../mod_node/mod_$modtype.php?lang=".LANG."&catpid=$catid&page=0&catid=$pidname>管理</a>";

 	$move= '<a class=but3 href='.$jumpv_back.'&file=submove&act=edit&pidname='.$pidname.'>转移</a>';
 	 if($last=='n') $move='';
...
?>
  ...
  <td align="center"><input type="text" name="<?php echo $tid;?>"  value="<?php echo $vsub['pos'];?>" size="5" /></td>
    ...
    <strong><?php echo decode($vsub['name']);?></strong>
     <br /><?php   echo $pidname;?>
     <br /><?php  if($modtype=='node')  echo $url_subcc;?><i class="fa fa-external-link-square"></i>
      ...
      //----if sub sub cat------------------------       
         if($row_sub<>'no'){
              foreach($row_sub as $v2_sub){
					...
					$edit_desp='<a class=but1 href='.$jumpv_back.'&file=subedit&act=edit&tid='.$subtid.'><i class="fa fa-pencil-square-o"></i> 修改</a>';
					 
					$del_text= " <a  class='but2' href=javascript:del('delsubcate','$subpidname','$jumpv_back','$jsname')><i class='fa fa-trash-o'></i> 删除</a>";
					//$gltext2 = 	"<a class='but1' href=../mod_node/mod_$modtype.php?lang=cn&catpid=$catid&page=0&catid=$subpidname>管理</a>";

					$move='<a class="but3" href='.$jumpv_back.'&file=submove&act=edit&pidname='.$subpidname.'>转移</a>';
					 if($last_sub=='n') $move='';
				  ...
				  <td align="center">&nbsp;&nbsp;├ <input type="text" name="<?php echo $subtid;?>"  value="<?php echo $v2_sub['pos'];?>" size="3" /></td>
					<td align="left" style="color:#999;padding-left:15px">
					 ...
					 <br /><?php   echo $subpidname;?>
					  <br /><?php if($modtype=='node')  echo $url_sub2;?><i class="fa fa-external-link-square"></i>
