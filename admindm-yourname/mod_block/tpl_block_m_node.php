 ...
 //$despjj = zbdespadd2($abc5);   
   ...
   // $desp = zbdesp_onlyinsert($_POST['despcontent']); //note: desp and addr not use variable abc1.
    ...
    $template = zbdesp_onlyinsert($_POST['template']); 
        ...
        if($abc5<1) $abc5=1;
        $ss = "update " . TABLE_BLOCK . " set name='$abc1',cssname='$abc2',pidcate='$abc3',template='$template',maxline='$abc5',cus_columns='$abc6',cus_substrnum='$abc7',namefront='$abc8',despjj='$abc9',bgcolor='$abc10',blockimg='$abc11',linktitle='$abc12',linkurl='$abc13' where pid='$type' and pidname='$pidname' $andlangbh limit 1"; 
        //echo $ss;exit;
        iquery($ss);  
...
if ($act == 'insert') {
   ...
   // $desptext = zbdesp_onlyinsert($_POST['editor1text']);
if($abc5<1) $abc5=1;
      $ss = "insert into ".TABLE_BLOCK." (pid,pidname,pbh,lang,name,cssname,pidcate,template,maxline,cus_columns,cus_substrnum,namefront,despjj,bgcolor,blockimg,linktitle,linkurl) values ('$type','$pidnamecur','$user2510','".LANG."','$abc1','$abc2','$abc3','$abc4','$abc5','$abc6','$abc7','$abc8','$abc9','$abc10','$abc11','$abc12','$abc13')";  
      //echo $ss;exit;
...
?> 
 ...
 <a class="needpopup" href="../mod_module/mod_showcateid.php?lang=<?php echo LANG?>">查看分类标识></a>  
 <br />
<?php
if($pidcate<>''){
 $catname = get_field(TABLE_CATE,'name',$pidcate,'pidname');
 if($catname=='noid') echo '<span style="color:red">分类标识不对</span>';
 else  echo spanred($catname);
}
