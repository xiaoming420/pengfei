...
?>
 ...
 if($pid=='') echo '请先在上面或左边选择';
		...
		if($alias=='index') echo '(提示，如果是首页，不在这里布局，<br />首页的布局由  模板里设置决定。<a target="_blank" href="'.$linkhere.'">点击进入> </a>)<br /><br />';
     	else {
					 ...
					 if(getnum($sqllayout)>0){   
					   ...
					   require_once HERE_ROOT.'mod_layout/tpl_layout_rg.php'; 
						...
						if($type=='common'){
							...
							$ss = "insert into ".TABLE_LAYOUT." (pid,pidname,pidstylebh,pbh,lang,type,sta_bread_posi,sta_sidebar,dateedit) values ('$pid','$pidname','$curstyle','$user2510','".LANG."','$type','r','l','$dateall')";
							iquery($ss);
