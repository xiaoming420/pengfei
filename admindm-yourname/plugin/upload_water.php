...
<?php
if($waterimg=='')
$waterimg = TMIMG;//default water is tm.gif
/******************************************************************************
使用说明:
...
//取得水印图像尺寸，信息
$lim_size = getimagesize($waterimg);
$src_lim ='';
switch($lim_size[2]){
    case 1:
        $src_lim=@imagecreatefromgif($waterimg);
        break;
    case 2:
  ...
  //  $src_lw = ($src_w-$lim_size[0])/2;  //水印位于背景图正中央width定位
   ...
   $src_lw =$src_w-$lim_size[0]; //水印位于背景图正中央width定位
    $src_lh =$src_h-$lim_size[1];  //height定位
